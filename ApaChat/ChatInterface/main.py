...
class AsyncTk(tk.Tk):
    ...
    async def auto_connect_saved(self):
        llm_data_json = None
        try:
            llm_data_json = keyring.get_password("ApaChat", "LLM")
        ...
        mcp_list_json = None
        try:
            mcp_list_json = keyring.get_password("ApaChat", "MCP_list")
        except Exception as e:
            ...
            if messagebox.askyesno("Delete Server", f"Are you sure you want to delete {server_url}?"):
                ...
                if keyring:
                    try:
                        ...
                        list_json = keyring.get_password("ApaChat", "MCP_list")
                        if list_json:
                ...
                if server_info.get("connected"):
                    ...
                    async def reconnect_and_open():
                        try:
                            ...
                            if keyring:
                                try:
                                    cred_json = keyring.get_password("ApaChat", f"MCP_{server_url}")
